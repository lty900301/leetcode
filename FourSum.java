 ...
 * @author joshluo
 *
 * My solution is to first store all possible tow sum. Then 4sum is the combination of two
 * 2sum. So to store, you need O(n^2) to find all combination. Now you have O(n^2) space to 
 * store these tow sums. In this O(n^2) space, you need to go through them all to find 4sum.
 * Therefore, this finding process also takes O(n^2)
 * 
...
public class FourSum {
	public ArrayList<ArrayList<Integer>> fourSum(int[] num, int target) {
        ...
        // O(n^2)
        HashMap<Integer, ArrayList<ArrayList<Integer>>> twoSumMap = 
        ...
        while(iter.hasNext()) {
            ...
            while(firstCombination.size() > 0) {
                ...
                if(twoSumMap.containsKey(target-firstTwoSum)) {
                    ArrayList<ArrayList<Integer>> secondCombination = 
                        twoSumMap.get(target-firstTwoSum);
                ...
                } else break; // you can not make 4sum using this 2sum
            }
        }
