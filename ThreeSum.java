 ...
 * the array which gives the sum of zero. Note: Elements in a triplet (a,b,c) must be in non-descending order. (ie, a ¡Ü
 * b ¡Ü c) The solution set must not contain duplicate triplets.
 * 
    ...
    public ArrayList<ArrayList<Integer>> threeSum(int[] num) {
        ...
        for (int i = 0; i < num.length;) {
            ...
            for (int j = i + 1; j < num.length;) {
                ...
                if (third < second) {
                    while (j < num.length && second == num[j])
                        j++;
                    continue;
                }
            ...
            while (i < num.length && first == num[i])
                i++;
        }
        return result;
    }
