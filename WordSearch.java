...
public class WordSearch {
    ...
    public boolean findSubWord(char[][] board, boolean[][] visited, 
            String subWord, int x, int y) {
        ...
        if(x - 1 >= 0 && !visited[x-1][y] && board[x-1][y] == subWord.charAt(0))
            if(findSubWord(board, visited, subWord.substring(1), x-1, y)) return true;
        if(x + 1 < board.length && !visited[x+1][y] && board[x+1][y] == subWord.charAt(0))
        	if (findSubWord(board, visited, subWord.substring(1), x+1, y)) return true;
        if(y - 1 >= 0 && !visited[x][y-1] && board[x][y-1] == subWord.charAt(0))
        	if(findSubWord(board, visited, subWord.substring(1), x, y-1)) return true;
