...
public class MedianOfTwoSortedArrays {
	public double findMedianSortedArrays(int A[], int B[]) {
        ...
        assert(m + n > 0);
        if(m == 0) return (n % 2 == 0) ? (B[n/2 - 1] + B[n/2]) / 2.0 : (double)B[n/2];
        if(n == 0) return (m % 2 == 0) ? (A[m/2 - 1] + A[m/2]) / 2.0 : (double)A[m/2];
        ...
        if((m+n)%2 == 1) return (double)firstMid;
    ...
    int findKthInTwoSortedArrays(int A[], int B[], int k){
        ...
        while(start <= end){
            int cur = (start + end) / 2;
            if(cur < k && k <= B.length + cur + 1 && 
                (k == cur + 1 || A[cur] >= B[k-cur-2]) && 
