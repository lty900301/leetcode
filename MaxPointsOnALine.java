 ...
 * @author joshluo
 * 
 *         Answer: Consider the slope of a line. And be careful of the points in the same position. O(n^2) algo, Only
 *         iterate afterwards.
 * 
 */
        ...
        for (int i = 0; i < points.length; i++) {
            ...
            for (int j = i + 1; j < points.length; j++) {
                ...
                if (x2 - x1 == 0 && y2 - y1 == 0) { // if it is a same point
                    ...
                    double slope = 0;
                    if (x2 - x1 == 0)
                        slope = Double.MAX_VALUE; // infinity slope
                    else if (y2 - y1 == 0)
                        slope = 0; // 0 slope, have to specify this, since using double
                    else
